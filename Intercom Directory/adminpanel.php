...
function displayAllUnits($units, $type = 'heads') {
    if (empty($units)) {
        return '';
    }
    ...
    $output = '';
    if ($type == 'heads') {
        $output .= '<div class="hierarchy-divider"></div>';
        $output .= '<div style="color: #38a169; font-size: 11px; margin-bottom: 5px;">Heads of:</div>';
    } else {
        $output .= '<div class="hierarchy-divider"></div>';
        $output .= '<div style="color: #4299e1; font-size: 11px; margin-bottom: 5px;">Staff of:</div>';
    }
    
    foreach ($units as $index => $unit) {
        if ($index >= 3) { // Limit to first 3 to avoid tooltip overflow
            $output .= '<div style="color: #a0aec0; font-size: 10px;">+' . (count($units) - 3) . ' more...</div>';
            break;
        }
        $output .= '<div style="color: white; font-size: 10px; margin-bottom: 2px;">• ' . 
                   htmlspecialchars($unit['unit_name']) . 
                   ' <span style="color: #a0aec0;">(' . htmlspecialchars($unit['unit_type']) . ')</span></div>';
    }
                        ...
                        <div class="hierarchy-tooltip">
                            ...
                            <div class="hierarchy-info">
                                ...
                                <?php if (!empty($user_hierarchy['current_unit'])): ?>
                                    <div class="hierarchy-row">
                                        <div class="hierarchy-label">Primary Unit:</div>
                                        <div class="hierarchy-value">
                                ...
                                <?php 
                                // Show all units for heads (if more than one)
                                if (!empty($user_hierarchy['all_units']) && is_array($user_hierarchy['all_units']) && count($user_hierarchy['all_units']) > 0): 
                                    if (count($user_hierarchy['all_units']) > 1) {
                                        echo '<div class="hierarchy-divider"></div>';
                                        echo '<div style="color: #38a169; font-size: 11px; margin-bottom: 5px;">Heads of:</div>';
                                        $count = 0;
                                        foreach ($user_hierarchy['all_units'] as $index => $unit): 
                                            if ($index >= 3): 
                                                echo '<div style="color: #a0aec0; font-size: 10px;">+' . (count($user_hierarchy['all_units']) - 3) . ' more...</div>';
                                                break;
                                            endif;
                                            if (!empty($unit['unit_name'])) {
                                                ?>
                                                <div style="color: white; font-size: 10px; margin-bottom: 2px;">
                                                    • <?php echo htmlspecialchars($unit['unit_name']); ?> 
                                                    <span style="color: #a0aec0;">(<?php echo htmlspecialchars($unit['unit_type'] ?? 'Unknown'); ?>)</span>
                                                </div>
                                                <?php 
                                            }
                                        endforeach;
                                ...
                                // Show all units for staff
                                if (!empty($user_hierarchy['staff_of']) && is_array($user_hierarchy['staff_of']) && count($user_hierarchy['staff_of']) > 0): 
                                    echo '<div class="hierarchy-divider"></div>';
                                    echo '<div style="color: #4299e1; font-size: 11px; margin-bottom: 5px;">Staff of:</div>';
                                    $count = 0;
                                    foreach ($user_hierarchy['staff_of'] as $index => $unit): 
                                        if ($index >= 3): 
                                            echo '<div style="color: #a0aec0; font-size: 10px;">+' . (count($user_hierarchy['staff_of']) - 3) . ' more...</div>';
                                            break;
                                        endif;
                                        if (!empty($unit['unit_name'])) {
                                            ?>
                                            <div style="color: white; font-size: 10px; margin-bottom: 2px;">
                                                • <?php echo htmlspecialchars($unit['unit_name']); ?> 
                                                <span style="color: #a0aec0;">(<?php echo htmlspecialchars($unit['unit_type'] ?? 'Unknown'); ?>)</span>
                                                <?php if (!empty($unit['head_name'])): ?>
                                                    <br><span style="color: #a0aec0; margin-left: 10px;">Head: <?php echo htmlspecialchars($unit['head_name']); ?></span>
                                                <?php endif; ?>
                                            </div>
                                            <?php 
                                        }
                                ...
                                // Show head info for staff if available and not already shown
                                if (empty($user_hierarchy['staff_of']) && !empty($user_hierarchy['head_info'])): 
                                    $head_info = $user_hierarchy['head_info'];
                                    if (!empty($head_info['head_name'])) {
                                        ?>
                                        <div class="hierarchy-divider"></div>
                                        <div class="hierarchy-row">
                                            <div class="hierarchy-label">Head:</div>
                                            <div class="hierarchy-value">
                                                <?php echo htmlspecialchars($head_info['head_name']); ?>
                                                <?php if (!empty($head_info['head_email'])): ?>
                                                    <br><span style="color: #a0aec0; font-size: 9px;"><?php echo htmlspecialchars($head_info['head_email']); ?></span>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                                        <?php 
...
<div class="content">
    <div class="container">
        <div class="sidebar">
            ...
            <h3>Other Online Users (<?php echo count($online_users); ?>)</h3>
            <div class="online-users-list">
                ...
                <?php if(!$has_online_users): ?>
                    <div style="color: #a0aec0; padding: 10px; text-align: center;">No other online users</div>
                <?php endif; ?>
            </div>
                                ...
                                <div class="hierarchy-info">
                                    ...
                                    <div class="hierarchy-row">
                                        ...
                                        <div class="hierarchy-value">
                                            ...
                                            <?php if (!empty($sender_hierarchy['is_head'])): ?>
                                                <span style="color: #38a169; font-weight: bold; margin-left: 5px;">(Head)</span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                    ...
                                    <?php 
                                    // Show all units for heads (if more than one)
                                    if (!empty($sender_hierarchy['all_units']) && is_array($sender_hierarchy['all_units']) && count($sender_hierarchy['all_units']) > 0): 
                                        if (count($sender_hierarchy['all_units']) > 1) {
                                            echo '<div class="hierarchy-divider"></div>';
                                            echo '<div style="color: #38a169; font-size: 11px; margin-bottom: 5px;">Heads of:</div>';
                                            $count = 0;
                                            foreach ($sender_hierarchy['all_units'] as $index => $unit): 
                                                if ($index >= 3): 
                                                    echo '<div style="color: #a0aec0; font-size: 10px;">+' . (count($sender_hierarchy['all_units']) - 3) . ' more...</div>';
                                                    break;
                                                endif;
                                                if (!empty($unit['unit_name'])) {
                                                    ?>
                                    ...
                                    // Show all units for staff
                                    if (!empty($sender_hierarchy['staff_of']) && is_array($sender_hierarchy['staff_of']) && count($sender_hierarchy['staff_of']) > 0): 
                                        echo '<div class="hierarchy-divider"></div>';
                                        echo '<div style="color: #4299e1; font-size: 11px; margin-bottom: 5px;">Staff of:</div>';
                                        $count = 0;
                                        foreach ($sender_hierarchy['staff_of'] as $index => $unit): 
                                            if ($index >= 3): 
                                                echo '<div style="color: #a0aec0; font-size: 10px;">+' . (count($sender_hierarchy['staff_of']) - 3) . ' more...</div>';
                                                break;
                                            endif;
                                            if (!empty($unit['unit_name'])) {
                                                ?>
                                    ...
                                    // Show head info for staff if available and not already shown
                                    if (empty($sender_hierarchy['staff_of']) && !empty($sender_hierarchy['head_info'])): 
                                        $head_info = $sender_hierarchy['head_info'];
                                        if (!empty($head_info['head_name'])) {
                                            ?>
                                            <div class="hierarchy-divider"></div>
                                            <div class="hierarchy-row">
                                    ...
                                    <td style="font-weight: 500; color: #2d3748;">
                                        ...
                                        <small style="color: #718096; font-size: 11px;">
                                            ...
                                            <?php if (!empty($user_hierarchy['current_unit'])): ?> 
                                                • <?php echo htmlspecialchars($user_hierarchy['current_unit']); ?>
                                                <?php if (!empty($user_hierarchy['unit_type'])): ?>
                                                    (<?php echo htmlspecialchars($user_hierarchy['unit_type']); ?>)
                                                <?php endif; ?>
                                            <?php endif; ?>
                                        </small>
                                    </td>
            ...
            <?php else: ?>
                <div class="no-chat-selected">
                    <div>
                        ...
                        <p>Select a user from the other online users list to start chatting</p>
                        <p>You can only chat with regular users, not other operators.</p>
                        <ul style="text-align: left; margin-top: 10px;">
                            <li>Other Online users</li>
