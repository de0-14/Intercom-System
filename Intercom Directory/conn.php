...
/**
 * Get all contacts where user is the head
 */
function getHeadContacts($conn, $user_id) {
    $sql = "SELECT n.*, d.division_name, dept.department_name, u.unit_name, o.office_name,
            ...
            LEFT JOIN (
                SELECT number_id, AVG(rating) as avg_rating, COUNT(*) as total_feedbacks 
                FROM feedback 
                GROUP BY number_id
...
/**
 * Get unread conversation count for head user across all their contacts
 */
function getHeadUnreadCount($conn, $head_user_id) {
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ii", $head_user_id, $head_user_id);
    $stmt->execute();
    $result = $stmt->get_result();
    $row = $result->fetch_assoc();
...
/**
 * Get unread conversation count for a specific contact where user is head
 */
function getHeadUnreadForContact($conn, $head_user_id, $number_id) {
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ii", $number_id, $head_user_id);
    $stmt->execute();
    $result = $stmt->get_result();
    $row = $result->fetch_assoc();
    ...
    // If user is staff (role_id 7), find their organizational unit and head
        ...
        if ($staff_result->num_rows > 0) {
            ...
            if ($staff_data['division_id']) {
                $hierarchy_info['division'] = $staff_data['division_name'];
            }
            if ($staff_data['department_id']) {
                $hierarchy_info['department'] = $staff_data['department_name'];
            }
            if ($staff_data['unit_id']) {
                $hierarchy_info['unit'] = $staff_data['unit_name'];
            }
            if ($staff_data['office_id']) {
                $hierarchy_info['office'] = $staff_data['office_name'];
            }
