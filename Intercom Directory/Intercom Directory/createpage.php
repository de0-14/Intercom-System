...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $checkStmt = $conn->prepare("SELECT id FROM divisions WHERE name = ?");
        ...
        // Insert division
        $stmt = $conn->prepare("INSERT INTO divisions (name, created_by) VALUES (?, ?)");
        $stmt->bind_param("si", $divisionName, $createdBy);
        ...
        if (isset($_POST['numbers'])) {
            $phoneStmt = $conn->prepare("INSERT INTO phone_numbers (division_id, phone_number, number_type) VALUES (?, ?, ?)");
                ...
                if (!empty($phoneNumber)) {
                    ...
                    if (isset($numberData['types'])) {
                        foreach ($numberData['types'] as $type) {
                            $phoneStmt->bind_param("iss", $divisionId, $phoneNumber, $type);
                ...
                formData.forEach((value, key) => {
                    ...
                    if (key.includes('[') && key.includes(']')) {
                        ...
                        if (matches) {
                            ...
                            if (!formObject[mainKey][subKey]) formObject[mainKey][subKey] = {};
                            formObject[mainKey][subKey][subSubKey] = value;
                        } else {
