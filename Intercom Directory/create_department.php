...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $underDivision = trim($_POST['under']); // FIXED: Define $underDivision
        ...
        if (empty($divisionhead)) {
            throw new Exception("Department Head is required.");
        }
        if (empty($underDivision)) {
            throw new Exception("Please select a division.");
        }

        // Check if department already exists
        $checkStmt = $conn->prepare("SELECT department_id FROM departments WHERE department_name = ?");
        $checkStmt->bind_param("s", $divisionName);
        $checkStmt->execute();
        $checkStmt->store_result();

        if ($checkStmt->num_rows > 0) {
            throw new Exception("Department '$divisionName' already exists.");
        }
        $checkStmt->close();
        ...
        $stmt->bind_param("si", $divisionName, $underDivision);
        ...
        if (isset($_POST['numbers'])) {
            $phoneStmt = $conn->prepare("INSERT INTO numbers (numbers, description, head, department_id) VALUES (?, ?, ?, ?)");
                ...
                if (!empty($phoneNumber)) {
                    ...
                    if (isset($numberData['types'])) {
                        foreach ($numberData['types'] as $type) {
                            $phoneStmt->bind_param("sssi", $phoneNumber, $type, $divisionhead, $departmentId);
        ...
        $response['message'] = "Department '$divisionName' created successfully!";
...
<body>
    ...
    <div class="big-container">
        <h2>You are creating a new Department</h2>
                ...
                <select id="division" name="under" required>
                    ...
                    <?php foreach ($divisions as $division): ?>
                        <option value="<?php echo $division['division_id']; ?>"
                            <?php echo isset($_POST['under']) && $_POST['under'] == $division['division_id'] ? 'selected' : ''; ?>>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            form.addEventListener('submit', function(e) {
                ...
                const underDivision = document.getElementById('division').value; // Get the selected division
                ...
                if (!divisionHead) {
                    showMessage('Department Head is required.');
                    return;
                }
                if (!underDivision) { // Check if division is selected
                ...
                formData.append('under', underDivision); // Add the division value
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById('division').value = ''; // Reset select
