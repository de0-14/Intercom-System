    ...
    if (isset($_POST['update_contact'])) {
        if (!$user_id) {
            ...
            if (empty($contact_number)) {
                ...
                $edit_error = "Head name is required.";
            } else {
                $update_sql = "UPDATE numbers SET numbers = ?, description = ?, head = ?, status = ? WHERE number_id = ? AND head_user_id = ?";
                $update_stmt = $conn->prepare($update_sql);
                $update_stmt->bind_param("ssssii", $contact_number, $description, $head_name, $status, $number_id, $user_id);
                ...
                <form method="POST" class="edit-form">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="head_name" name="head_name" class="form-control" 
                               value="<?php echo htmlspecialchars($contact['head']); ?>" required>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.messaging-section textarea, .message-form textarea').forEach(textarea => {
        ...
        // Auto-focus if not viewing archived
        <?php if (!$view_archived): ?>
