...
// Get head user ID - we need it for permission checks
...
// ==========================================
// AUTO ARCHIVE ON PAGE LOAD WITH READ CHECK
// ==========================================
$archived_this_load = 0;
if ($user_id) {
    // Archive conversations that are READ and inactive for 30+ minutes
    // OR unread conversations inactive for 60+ minutes (longer grace period)
    
    $thirty_minutes_ago = date('Y-m-d H:i:s', strtotime('-5 minutes'));
    $sixty_minutes_ago = date('Y-m-d H:i:s', strtotime('-60 minutes'));
    
    // Get conversations for this number that meet archive criteria
    $inactive_sql = "
        ...
        AND c.is_archived = 0 
        AND (
            -- CASE 1: All messages are READ AND last activity > 30 minutes
            (c.last_activity < ? AND NOT EXISTS (
                SELECT 1 FROM messages m2 
                WHERE m2.conversation_id = c.conversation_id 
                AND m2.is_read = 0
                AND m2.receiver_id = c.initiated_by
            ))
            OR
            -- CASE 2: Has UNREAD messages but > 60 minutes inactive
            (c.last_activity < ? AND EXISTS (
                SELECT 1 FROM messages m3 
                WHERE m3.conversation_id = c.conversation_id 
                AND m3.is_read = 0
                AND m3.receiver_id = c.initiated_by
            ))
        )
    ";
    
    $inactive_stmt = $conn->prepare($inactive_sql);
    $inactive_stmt->bind_param("iss", $number_id, $thirty_minutes_ago, $sixty_minutes_ago);
...
// Check if user has any archived conversations (for button display)
$archive_count_for_button = 0;
if ($user_id) {
    if ($is_head) {
        $check_archived_stmt = $conn->prepare("
            SELECT COUNT(*) as archive_count 
            FROM conversations_archive 
            WHERE number_id = ?
        ");
        $check_archived_stmt->bind_param("i", $number_id);
    } else {
        $check_archived_stmt = $conn->prepare("
            SELECT COUNT(*) as archive_count 
            FROM conversations_archive 
        ...
        ");
        $check_archived_stmt->bind_param("ii", $number_id, $user_id);
    }
    $check_archived_stmt->execute();
    $archive_result = $check_archived_stmt->get_result();
    $archive_data = $archive_result->fetch_assoc();
    $archive_count_for_button = $archive_data['archive_count'] ?? 0;
    $check_archived_stmt->close();
}
    ...
    if (isset($_POST['send_message'])) {
        if (!$user_id) {
            ...
            if (empty($message)) {
                ...
                // Get head user ID for this contact - USE THE ORIGINAL WAY
                $head_user_id = getHeadUserId($conn, $number_id);
...
if ($user_id) {
    ...
    if ($selected_conversation_id) {
        ...
        if ($access_result->num_rows === 1) {
            ...
            if ($selected_conversation_id && $user_id) {
...
<head>
    ...
    /* Simple style for the rest - removed complex archive styles */
    </style>
</head>
<body>
...
<!-- TEST ARCHIVE BUTTON (always show) -->
<div class="archive-button-section">
    <a href="view_archived.php?number_id=<?php echo $number_id; ?>" 
       class="archive-view-btn">
        <span class="archive-icon">📁</span>
        View Archived Conversations
    </a>
    <?php if ($archive_count_for_button > 0): ?>
        <span style="color: #666; font-size: 12px; margin-left: 10px;">
            (<?php echo $archive_count_for_button; ?> archived conversations)
        </span>
    <?php endif; ?>
</div>
