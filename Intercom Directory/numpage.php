...
if ($user_id) {
    ...
    if ($selected_conversation_id && !$view_archived) {
        ...
        $access_check_params = array($user_id, $user_id, $selected_conversation_id, $user_id, $head_user_id, $user_id);
        ...
        if ($access_check && sqlsrv_has_rows($access_check)) {
            ...
            $mark_read_sql = "
                ...
                AND is_archived = 0
            ";
            $mark_read_params = array($selected_conversation_id, $user_id);
            sqlsrv_query($conn, $mark_read_sql, $mark_read_params);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update_contact'])) {
        if (!$user_id) {
            ...
            if (empty($contact_number)) {
                ...
                $update_params = array($contact_number, $description, $status, $number_id, $user_id);
    ...
    if (isset($_POST['submit_feedback']) || (isset($_POST['ajax']) && $_POST['ajax'] === 'true' && isset($_POST['rating']) && isset($_POST['comment']))) {
        ...
        if (!$user_id) {
            ...
            $check_feedback_sql = "SELECT feedback_id FROM feedback WHERE number_id = ? AND user_id = ?";
            $check_feedback_params = array($number_id, $user_id);
            $check_feedback_stmt = sqlsrv_prepare($conn, $check_feedback_sql, $check_feedback_params);
            
            if ($check_feedback_stmt && sqlsrv_execute($check_feedback_stmt)) {
                if (sqlsrv_has_rows($check_feedback_stmt)) {
                    ...
                    $insert_feedback_sql = "INSERT INTO feedback (number_id, user_id, rating, comment, created_at, updated_at) VALUES (?, ?, ?, ?, GETDATE(), GETDATE())";
                    $insert_feedback_params = array($number_id, $user_id, $rating, $comment);
                    $insert_feedback_stmt = sqlsrv_prepare($conn, $insert_feedback_sql, $insert_feedback_params);
                    
                    if ($insert_feedback_stmt && sqlsrv_execute($insert_feedback_stmt)) {
                    ...
                    sqlsrv_free_stmt($insert_feedback_stmt);
                }
            }
            sqlsrv_free_stmt($check_feedback_stmt);
        }
    }
    ...
    if (isset($_POST['update_feedback']) || (isset($_POST['ajax']) && $_POST['ajax'] === 'true' && isset($_POST['feedback_id']) && isset($_POST['rating']) && isset($_POST['comment']))) {
        if (!$user_id) {
            ...
            $verify_feedback_sql = "SELECT feedback_id FROM feedback WHERE feedback_id = ? AND user_id = ?";
            $verify_feedback_params = array($feedback_id, $user_id);
            $verify_feedback_stmt = sqlsrv_prepare($conn, $verify_feedback_sql, $verify_feedback_params);
            
            if ($verify_feedback_stmt && sqlsrv_execute($verify_feedback_stmt)) {
                if (!sqlsrv_has_rows($verify_feedback_stmt)) {
                    ...
                    $update_feedback_sql = "UPDATE feedback SET rating = ?, comment = ?, updated_at = GETDATE() WHERE feedback_id = ? AND user_id = ?";
                    $update_feedback_params = array($rating, $comment, $feedback_id, $user_id);
                    $update_feedback_stmt = sqlsrv_prepare($conn, $update_feedback_sql, $update_feedback_params);
                    
                    if ($update_feedback_stmt && sqlsrv_execute($update_feedback_stmt)) {
                        // If AJAX request, return JSON
                        if ($is_ajax) {
                    ...
                    sqlsrv_free_stmt($update_feedback_stmt);
                }
            }
            sqlsrv_free_stmt($verify_feedback_stmt);
        }
    }
        ...
        if (!$user_id) {
            ...
            if ($verify_feedback_stmt && sqlsrv_execute($verify_feedback_stmt)) {
                if (!sqlsrv_has_rows($verify_feedback_stmt)) {
                    ...
                    $delete_feedback_sql = "DELETE FROM feedback WHERE feedback_id = ? AND user_id = ?";
                    $delete_feedback_params = array($feedback_id, $user_id);
                    $delete_feedback_stmt = sqlsrv_prepare($conn, $delete_feedback_sql, $delete_feedback_params);
                    
                    if ($delete_feedback_stmt && sqlsrv_execute($delete_feedback_stmt)) {
                        if ($is_ajax) {
                    ...
                    sqlsrv_free_stmt($delete_feedback_stmt);
                }
            }
            sqlsrv_free_stmt($verify_feedback_stmt);
        }
        ...
        if (!$user_id) {
            ...
            if (empty($message)) {
                ...
                if (!$head_user_id) {
                    ...
                    $check_conversation_sql = "
                        SELECT TOP 1 conversation_id FROM conversations 
                        WHERE number_id = ? AND initiated_by = ?
                        AND is_archived = 0
                    ";
                    $check_conversation_params = array($number_id, $user_id);
                    $check_conversation_stmt = sqlsrv_prepare($conn, $check_conversation_sql, $check_conversation_params);
                    
                    if ($check_conversation_stmt && sqlsrv_execute($check_conversation_stmt)) {
                        if (sqlsrv_has_rows($check_conversation_stmt)) {
                            $conv_row = sqlsrv_fetch_array($check_conversation_stmt, SQLSRV_FETCH_ASSOC);
                            $conversation_id = $conv_row['conversation_id'];
                            sqlsrv_free_stmt($check_conversation_stmt);
                        } else {
                            sqlsrv_free_stmt($check_conversation_stmt);
                            ...
                            ";
                            $create_conversation_params = array($number_id, $user_id);
                            $create_conversation_stmt = sqlsrv_prepare($conn, $create_conversation_sql, $create_conversation_params);
                            
                            if ($create_conversation_stmt && sqlsrv_execute($create_conversation_stmt)) {
                                $conversation_id = getLastInsertId($conn);
                            } else {
                                $message_error = "Failed to start conversation. Please try again.";
                                $conversation_id = null;
                            }
                            sqlsrv_free_stmt($create_conversation_stmt);
                        }
                        ...
                        if ($conversation_id) {
                            if (empty($head_user_id) || $head_user_id <= 0) {
                                ...
                                $insert_message_sql = "
                                    ...
                                    VALUES (?, ?, ?, ?, ?, GETDATE())
                                ";
                                $insert_message_params = array($user_id, $head_user_id, $number_id, $conversation_id, $message);
                                $insert_message_stmt = sqlsrv_prepare($conn, $insert_message_sql, $insert_message_params);
                                
                                if ($insert_message_stmt && sqlsrv_execute($insert_message_stmt)) {
                                    ...
                                    $message_error = "Failed to send message. SQL Error: " . print_r(sqlsrv_errors(), true);
                                    if ($is_ajax) {
                                        echo json_encode(['success' => false, 'error' => $message_error]);
                                        exit;
                                    }
                                }
                                sqlsrv_free_stmt($insert_message_stmt);
                            }
                        }
                    } else {
                        $message_error = "Failed to check conversation. SQL Error: " . print_r(sqlsrv_errors(), true);
                        if ($is_ajax) {
                            echo json_encode(['success' => false, 'error' => $message_error]);
                            exit;
                        }
        ...
        if (!$user_id) {
            ...
            if (empty($reply_message)) {
                ...
                $conv_info_sql = "
                    ...
                    AND c.is_archived = 0
                ";
                $conv_info_params = array($conversation_id);
                $conv_info_stmt = sqlsrv_prepare($conn, $conv_info_sql, $conv_info_params);
                
                if ($conv_info_stmt && sqlsrv_execute($conv_info_stmt)) {
                    if (sqlsrv_has_rows($conv_info_stmt)) {
                        $conv_data = sqlsrv_fetch_array($conv_info_stmt, SQLSRV_FETCH_ASSOC);
                        ...
                        $insert_reply_sql = "
                            ...
                            VALUES (?, ?, ?, ?, ?, GETDATE())
                        ";
                        $insert_reply_params = array($user_id, $receiver_id, $number_id, $conversation_id, $reply_message);
                        $insert_reply_stmt = sqlsrv_prepare($conn, $insert_reply_sql, $insert_reply_params);
                        
                        if ($insert_reply_stmt && sqlsrv_execute($insert_reply_stmt)) {
                            updateConversationActivity($conn, $conversation_id);
                            ...
                            $message_error = "Failed to send reply. Error: " . print_r(sqlsrv_errors(), true);
                            if ($is_ajax) {
                                echo json_encode(['success' => false, 'error' => $message_error]);
                                exit;
                            }
                        }
                        sqlsrv_free_stmt($insert_reply_stmt);
                    } else {
                    ...
                    $message_error = "Failed to get conversation info. Error: " . print_r(sqlsrv_errors(), true);
                    if ($is_ajax) {
                        echo json_encode(['success' => false, 'error' => $message_error]);
                        exit;
                    }
                }
                sqlsrv_free_stmt($conv_info_stmt);
            }
        }
    }
}
                            ...
                            <?php foreach ($conversations as $conv): ?>
                                <tr>
                                    ...
                                    <td class="archived-date">
                                        <?php echo formatDateForDisplay($conv['conversation_start']); ?>
                                    </td>
                                    <td class="archived-date">
                                        <?php echo formatDateForDisplay($conv['last_message_time']); ?>
                                    </td>
                                    <td class="archived-date">
                                        <?php echo formatDateForDisplay($conv['archived_at']); ?>
                                    </td>
                                    <td>
                        ...
                        <?php foreach ($conversations as $conv): ?>
                            <a href="<?php echo $current_script; ?>?id=<?php echo $number_id; ?>&conversation_id=<?php echo $conv['conversation_id']; ?>" 
                               class="conversation-item <?php echo ($selected_conversation_id == $conv['conversation_id']) ? 'active' : ''; ?>">
                                ...
                                <?php 
                                // Get last message for preview (SQL Server version)
                                $preview_sql = "
                                ...
                                $preview_stmt = sqlsrv_prepare($conn, $preview_sql, $preview_params);

                                if ($preview_stmt && sqlsrv_execute($preview_stmt)) {
                                    $last_message = sqlsrv_fetch_array($preview_stmt, SQLSRV_FETCH_ASSOC);
                                } else {
                                    $last_message = false;
                                }
                                if ($preview_stmt) {
