        ...
        if ($action === 'update_profile') {
            ...
            if (empty($error)) {
                ...
                if (mysqli_stmt_num_rows($check_stmt) > 0) {
                    ...
                    // Build update query with organizational fields
                    $update_sql = "UPDATE users SET username = ?, email = ?, full_name = ?, role_id = ?, division_id = ?, department_id = ?, unit_id = ?, office_id = ?";
                    $types = "sssiiiii";
                    $params = [$edit_username, $edit_email, $edit_fullname, $edit_role_id, $division_id, $department_id, $unit_id, $office_id];
                    ...
                    if (mysqli_stmt_execute($update_stmt)) {
                        ...
                        if ($edit_role_id == 7) { // Staff user
                            ...
                            mysqli_stmt_bind_param($clear_stmt, "i", $edit_user_id);
            ...
            if (empty($error)) {
                ...
                if (mysqli_stmt_num_rows($check_stmt) > 0) {
                    ...
                    mysqli_stmt_bind_param($insert_stmt, "sssisiiii", 
                        $edit_username, 
                        $edit_email, 
                        $edit_fullname, 
                        $edit_role_id, 
            ...
            if ($delete_user_id != $user_id) { // Prevent self-deletion
                ...
                if (mysqli_stmt_execute($update_stmt)) {
                    ...
                    $clear_sql = "UPDATE numbers SET head_user_id = NULL, head = NULL WHERE head_user_id = ?";
                    $clear_stmt = mysqli_prepare($conn, $clear_sql);
                    mysqli_stmt_bind_param($clear_stmt, "i", $delete_user_id);
                    mysqli_stmt_execute($clear_stmt);
                    mysqli_stmt_close($clear_stmt);
