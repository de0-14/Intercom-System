...
// Get all users for head assignment dropdown
$all_users_for_head = [];
$users_sql = "SELECT user_id, username, full_name, role_id FROM users WHERE status = 'active' ORDER BY full_name";
$users_result = $conn->query($users_sql);
while ($row = $users_result->fetch_assoc()) {
    $all_users_for_head[] = $row;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
            ...
            if (empty($error)) {
                ...
                if (mysqli_stmt_num_rows($check_stmt) > 0) {
                    ...
                    if (mysqli_stmt_execute($update_stmt)) {
                        ...
                        // Update numbers table head_user_id for numbers assigned to this organizational unit
                        if ($edit_division_id) {
                            // Find numbers in this division and update head_user_id
                            $update_numbers_sql = "UPDATE numbers SET head_user_id = ? WHERE division_id = ?";
                            $update_numbers_stmt = mysqli_prepare($conn, $update_numbers_sql);
                            mysqli_stmt_bind_param($update_numbers_stmt, "ii", $edit_user_id, $edit_division_id);
                            mysqli_stmt_execute($update_numbers_stmt);
                            mysqli_stmt_close($update_numbers_stmt);
                        }
                        
                        if ($edit_department_id) {
                            // Find numbers in this department and update head_user_id
                            $update_numbers_sql = "UPDATE numbers SET head_user_id = ? WHERE department_id = ?";
                            $update_numbers_stmt = mysqli_prepare($conn, $update_numbers_sql);
                            mysqli_stmt_bind_param($update_numbers_stmt, "ii", $edit_user_id, $edit_department_id);
                            mysqli_stmt_execute($update_numbers_stmt);
                            mysqli_stmt_close($update_numbers_stmt);
                        }
                        
                        if ($edit_unit_id) {
                            // Find numbers in this unit and update head_user_id
                            $update_numbers_sql = "UPDATE numbers SET head_user_id = ? WHERE unit_id = ?";
                            $update_numbers_stmt = mysqli_prepare($conn, $update_numbers_sql);
                            mysqli_stmt_bind_param($update_numbers_stmt, "ii", $edit_user_id, $edit_unit_id);
                            mysqli_stmt_execute($update_numbers_stmt);
                            mysqli_stmt_close($update_numbers_stmt);
                        }
                        
                        if ($edit_office_id) {
                            // Find numbers in this office and update head_user_id
                            $update_numbers_sql = "UPDATE numbers SET head_user_id = ? WHERE office_id = ?";
                            $update_numbers_stmt = mysqli_prepare($conn, $update_numbers_sql);
                            mysqli_stmt_bind_param($update_numbers_stmt, "ii", $edit_user_id, $edit_office_id);
                            mysqli_stmt_execute($update_numbers_stmt);
                            mysqli_stmt_close($update_numbers_stmt);
                        }
        ...
        } elseif ($action === 'assign_head' && isAdmin()) {
            // Admin can manually assign head for specific number
            $number_id = (int)$_POST['number_id'];
            $head_user_id = (int)$_POST['head_user_id'];
            
            $update_sql = "UPDATE numbers SET head_user_id = ? WHERE number_id = ?";
            $update_stmt = mysqli_prepare($conn, $update_sql);
            mysqli_stmt_bind_param($update_stmt, "ii", $head_user_id, $number_id);
            
            if (mysqli_stmt_execute($update_stmt)) {
                $success = "Head assigned successfully to number!";
            } else {
                $error = "Failed to assign head: " . mysqli_error($conn);
            }
            mysqli_stmt_close($update_stmt);
...
$all_users = [];
// Get all numbers for head management
$all_numbers = [];
if (isAdmin()) {
    $users_sql = "SELECT u.*, 
                     d.division_name, 
    ...
    // Get all numbers for head assignment
    $numbers_sql = "SELECT n.*, 
                    d.division_name,
                    dept.department_name,
                    un.unit_name,
                    o.office_name,
                    uh.username as head_username,
                    uh.full_name as head_full_name
             FROM numbers n
             ...
             LEFT JOIN users uh ON n.head_user_id = uh.user_id
             WHERE n.status = 'active'
             ORDER BY n.numbers";
    $numbers_result = $conn->query($numbers_sql);
...
function openUserModal(action, userId = null) {
    ...
    if (action === 'add') {
        ...
        if (user) {
            ...
            // Note: department_id and unit_id will be loaded asynchronously
            // So we need to handle this carefully
            if (user.department_id && !user.division_id) {
                // If department_id exists but division_id doesn't, this is an error
                console.warn('User has department_id but no division_id');
            }
            
            if (user.unit_id && !user.department_id) {
                console.warn('User has unit_id but no department_id');
            }
