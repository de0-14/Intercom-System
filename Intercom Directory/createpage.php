...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Get division name and head user ID
        ...
        if (empty($headUserId)) throw new Exception("Division Head is required.");
        ...
        // Process phone numbers - FIXED VERSION
        if (isset($_POST['numbers']) && is_array($_POST['numbers'])) {
            ...
            foreach ($_POST['numbers'] as $index => $numberData) {
                // Make sure the array structure is correct
                if (isset($numberData['number'])) {
                    $phoneNumber = trim($numberData['number']);
                    $description = isset($numberData['description']) ? trim($numberData['description']) : '';
                    
                    if (!empty($phoneNumber)) {
                        if (empty($description)) {
                            $description = "Division contact";
                        }
                        
                        $phoneStmt->bind_param("ssisi", $phoneNumber, $description, $headUserId, $headFullName, $divisionId);
...
.big-container h2 { color:#2b6cb0; margin-bottom:20px; }
...
<div class="content">
    ...
    <div class="big-container">
        <h2 class="center-text">Create a New Division</h2>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function createNumberEntry() {
        const entryId = numberCounter++; // Use counter directly
    ...
    form.addEventListener('submit', function(e){
        ...
        if(!name){ showMessage('Division name is required.'); return; }
        if(!head){ showMessage('Division Head is required.'); return; }
        ...
        // Simpler approach - use flat array structure
        numberEntries.forEach((entry) => {
            const numberInput = entry.querySelector('input[placeholder*="contact number"]');
            ...
            if(phone){
                hasValid = true;
                // Use simpler naming that PHP can easily handle
                formData.append(`numbers[${numberIndex}][number]`, phone);
                formData.append(`numbers[${numberIndex}][description]`, description || '');
                numberIndex++;
            }
        });
        ...
        .then(res => {
            ...
            return res.json().catch(err => {
                ...
                console.error('Response text:', res.text()); // Log response text
