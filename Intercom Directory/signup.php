...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check existing user/email using SQL Server
    if(empty($error)) {
        ...
        if ($stmt === false) {
            ...
            if (sqlsrv_has_rows($stmt)) {
                ...
                if (!$valid_hierarchy) {
                    ...
                    // Insert user with organizational data using SQL Server
                    $insert_sql = "INSERT INTO users (username, email, full_name, password, role_id, division_id, department_id, unit_id, office_id, status) 
                                   VALUES (?, ?, ?, ?, 7, ?, ?, ?, ?, 'active')";
                    $params2 = array($username, $email, $fullname, $password_hash, $division_id, $department_id, $unit_id, $office_id);
                    $stmt2 = sqlsrv_query($conn, $insert_sql, $params2);
                    ...
                    if($stmt2) {
                        ...
                        $error = "Error creating account: " . print_r(sqlsrv_errors(), true);
                    }
...
function loadOffices(unitId) {
    ...
    // Use GET request instead of POST
    fetch(`ajax_get_offices.php?unit_id=${unitId}`)
    .then(response => response.text())
    .then(data => {
