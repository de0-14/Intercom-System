...
echo "<!-- DEBUG: Is Admin = " . (isAdmin() ? 'YES' : 'NO') . " -->";

$allNumbers = getAllContactNumbers($conn);
echo "<!-- DEBUG: Number count = " . count($allNumbers) . " -->";
...
if(!empty($head_contacts)) {
    foreach($head_contacts as $head_contact) {
        $headNumberIds[$head_contact['number_id']] = true;
    }
}

foreach($allNumbers as $number) {
    if(isset($headNumberIds[$number['number_id']])) {
        continue;
    }
...
<div class="content">
    <div class="contact-directory">
            ...
            <?php if($user_id && !$is_admin): ?>
                ...
                <div style="position: relative; display: inline-block; margin-right: 10px;">
                    ...
                    <div style="padding: 8px 12px; background-color: #38a169; color: white; border-radius: 6px; font-size: 14px; font-weight: 500;">
                        <span class="pinned-icon">📌</span> My Contacts
                    </div>
                </div>
                <?php endif; ?>
            <?php endif; ?>
        ...
        <div class="stats-summary">
            ...
            <div class="stat-item"><div class="stat-value"><?php echo count($head_contacts); ?></div><div class="stat-label">My Contacts</div></div>
            <?php endif; ?>
        </div>
...
function filterSearch(){
    ...
    const rows = document.querySelectorAll('.contact-row, .head-contact-row');
