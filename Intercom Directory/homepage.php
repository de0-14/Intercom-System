...
function getAllContactNumbers($conn) {
    ...
    while($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
        ...
        // Set status based on number's own status field first, then fallback to organizational status
        if (isset($row['number_status']) && !empty($row['number_status'])) {
            $row['status'] = $row['number_status'];
        } else {
            // Fallback to organizational status if number status not available
